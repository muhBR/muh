...
RSpec.describe Mutations::ServiceOrder::UpdateServiceOrder, type: :request do
  let!(:user) { create(:user) }
  let!(:service_order) { create(:service_order, user: user) }
  let!(:customer) { create(:customer) }
  let!(:items) { create_list(:item, 2) }

  let!(:item_service_order1) do
    create(:item_service_order, service_order: service_order, item: items[0], quantity: 10, purchase_price: items[0].purchase_price, sale_price: items[0].sale_price )
  end

  let!(:item_service_order2) do
    create(:item_service_order, service_order: service_order, item: items[1], quantity: 20, purchase_price: items[1].purchase_price, sale_price: items[1].sale_price )
  end
  ...
  describe 'when data is valid' do
    ...
    subject { json_response('updateServiceOrder') }

    it 'returns serviceOrder data' do
      expect(subject).to include(
        'id' => service_order.id.to_s,
        'name' => name,
        'status' => status,
